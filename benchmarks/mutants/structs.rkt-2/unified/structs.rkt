(module structs racket/base
  (#%module-begin
   ...
   (define (new-suffix-tree) (make-suffix-tree (let ((root (make-node (make-label (make-vector 0.0)) #f (list) #f))) root)))
   ...
   (define (node-find-child node label-element)
     (define (loop children) (cond ((null? children) #f) ((label-element-equal? label-element (label-ref (node-up-label (first children)) 0)) (first children)) (else (loop (rest children)))))
